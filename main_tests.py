...
while True:
    ...
    if enable_STT:
        if stt_response["success"]:
            ...
            print("STT failed.")
            print(f"STT error: {stt_response['error']}")
            user_input_text = ""
    else:
        user_input_text = input("You: ")
        print(f"You typed: {user_input_text}")
    ...
    if end_interaction:
        ...
        llm_response_text = llm.request_response(user_input_text, role)
        print(f"LLM: {llm_response_text}")
    ...
    if enable_blossom:
        ...
        bl_thread = threading.Thread(
            ...
            kwargs={"audio_length": audio_length}
        )
        bl_thread.start()
